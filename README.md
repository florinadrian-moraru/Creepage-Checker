...
Performance (v189.1–v189.2): 
Added profiling instrumentation to find the next bottleneck, which revealed that edge_cuts_bboxes was still a full unindexed linear scan inside get_grid_mask — v189.0 had indexed obstacle_polygons but missed this separate check, accounting for ~120s of a ~386s profiled run (260 million Contains() calls). Indexed it the same way, and memoized get_is_portal (previously called 11 million times with no caching). Net effect: pathfinding time roughly halved again beyond v189.0's initial improvement.
